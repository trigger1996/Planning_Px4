...
bool px4_msgs__msg__rover_differential_guidance_status__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // pid_heading_integral
    PyObject * field = PyObject_GetAttrString(_pymsg, "pid_heading_integral");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->pid_heading_integral = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
...
PyObject * px4_msgs__msg__rover_differential_guidance_status__convert_to_py(void * raw_ros_message)
{
  ...
  {  // pid_heading_integral
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->pid_heading_integral);
    {
      int rc = PyObject_SetAttrString(_pymessage, "pid_heading_integral", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
